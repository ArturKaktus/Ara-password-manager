        ...
        foreach (var recordJson in AppDocument.CurrentDatabaseModel.RecordsArrayList.Select(recordModel => new JsonObject()
                 {
                     ...
                     { "url", recordModel.Url },
                     { "loginSymbol", SymbolModelHelper.GetStringBySymbolValue(recordModel.AfterLoginSymbol)},
                     { "passwordSymbol", SymbolModelHelper.GetStringBySymbolValue(recordModel.AfterPasswordSymbol)},
                     { "urlSymbol", SymbolModelHelper.GetStringBySymbolValue(recordModel.AfterUrlSymbol) }
                 }))
        {
